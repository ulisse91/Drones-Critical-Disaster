...
function run_simulation {
    ...
    depots=$1
    for val in ${array[@]}; do
        for nodes in {10,20,30,50,75,100,125,150,175,200}; do #
            for seed in {0..19}; do
                timestamp=$(date +%s)
            ...
            i=$(($i +1))
            echo -ne " $(echo "scale=0; $i*10" | bc -q)% \r"
        done
    done
    echo -e "[TERMINATED]\n"
}
...
run_simulation 2 "${StringArray_2[@]}"
run_simulation_wadd 2 "${StringArray_2[@]}"

declare -a StringArray_3=("drones_q3_d3.csv" "drones_q6_d3.csv" "drones_q9_d3.csv" "drones_q12_d3.csv")
run_simulation 3 "${StringArray_3[@]}"
run_simulation_wadd 3 "${StringArray_3[@]}"

declare -a StringArray_4=("drones_q4_d4.csv" "drones_q8_d4.csv" "drones_q12_d4.csv" "drones_q15_d4.csv")
run_simulation 4 "${StringArray_4[@]}"
run_simulation_wadd 4 "${StringArray_4[@]}"
